...
const ArticleLists = ({ articles, articlesLength, isPagination = true, isLength = true }: { articles: ArticleProps[]; articlesLength?: number; isPagination?: boolean; isLength?: boolean }) => {
  ...
  return (
    <section className={cn(`bg-green-600 ${isLength ? "px-4 md:px-10 lg:px-20 xl:px-32" : "px-0"} ${isPagination ? "pb-40" : "pb-32"} flex flex-col justify-start gap-5`)}>
      {/* Article length */}
      {isLength ? (
      ...
      {/* Article Card Lists */}
      <div className="bg-fuchsia-500 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[24px]">
